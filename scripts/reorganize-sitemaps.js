...
urls.forEach(url => {
  ...
  else if (/^\/[a-z-]+\/[a-z-]+$/i.test(pathname)) {
    categories.push({ ...url, type: "subcategory" });
  }
  else if (/^\/[a-z-]+$/i.test(pathname) && !STATIC_PATHS.has(pathname)) {
...
const totalUrls = staticPages.length + blogPages.length + categories.length + posts.length + caseStudies.length;
