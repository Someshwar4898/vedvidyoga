...
const API_BASE = (process.env.NEXT_PUBLIC_WP_API_URL ?? "https://api.vedvidyoga.com").replace(/\/+$/,"");
const SITE_URL = (process.env.NEXT_PUBLIC_SITE_URL ?? "https://vedvidyoga.com").replace(/\/+$/,"");
...
async function getAdditionalPaths() {
  ...
  console.log("\n📊 Sitemap Summary:");
  console.log(`   🏠 Static pages: ${getStaticPages().length}`);
  console.log(`   📝 Posts: ${posts.length}`);
  console.log(`   💼 Case studies: ${caseStudies.length}`);
  console.log(`   📁 Categories: ${categories.length}`);

  return [...categories, ...posts, ...caseStudies];
}
