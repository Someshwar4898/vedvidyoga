...
export function useCaseStudies({ initialCaseStudies } = {}) {
  const hasInitialCaseStudies = Array.isArray(initialCaseStudies);
  const [caseStudies, setCaseStudies] = useState(initialCaseStudies ?? _cache ?? []);
  const [loading, setLoading] = useState(!_cache && !hasInitialCaseStudies);
  const [error, setError] = useState(null);

  useEffect(() => {
    if (hasInitialCaseStudies) {
    ...
    if (_cache) {
      setCaseStudies(_cache);
      setLoading(false);
      return;
    }

    loadCaseStudies()
      .then((items) => setCaseStudies(items))
