...
// }
  ...
  function CaseStudyDetail({ initialCaseStudies, initialCaseStudy }) {
  const { slug } = useParams();
  const { caseStudies, loading } = useCaseStudies({
    initialCaseStudies: initialCaseStudies ?? (initialCaseStudy ? [initialCaseStudy] : undefined),
  });

  const currentIndex = caseStudies.findIndex((c) => c.slug === slug);
  const cs = initialCaseStudy ?? caseStudies[currentIndex];
