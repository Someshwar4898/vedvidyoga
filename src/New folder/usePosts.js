  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (isWPConnected()) {
          data = await getPosts({ categorySlug });
        ...
        // Filter client-side for subcategory
        if (subcategorySlug) {
          data = data.filter(
        ...
        if (!cancelled) {
          ...
          let data = mockPosts;
          if (subcategorySlug) data = data.filter((p) => p.categorySlug === categorySlug && p.subcategorySlug === subcategorySlug);
