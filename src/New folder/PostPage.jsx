...
function PostPage() {
  ...
  const { posts, loading } = usePosts({ categorySlug: category, subcategorySlug: subcategory });

  const post = posts.find(
    (p) => p.categorySlug === category && p.subcategorySlug === subcategory && p.slug === slug
  );

  if (loading) {
    return (
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-10 py-24 flex justify-center">
    ...
    <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-10 py-10 md:py-14">
      <div className="max-w-3xl mx-auto">
        ...
        <nav className="flex items-center gap-2 text-sm text-stone-400 dark:text-stone-500 mb-10">
          ...
          <span>›</span>
          <Link to={`/${post.categorySlug}/${post.subcategorySlug}`} className="hover:text-saffron dark:text-stone-400 transition">{post.subcategoryName}</Link>
        </nav>
        ...
        <div className="mt-16 pt-10 border-t border-[#f0e3d3] dark:border-stone-700">
          ...
          >
            <ArrowLeft size={16} /> Back to {post.subcategoryName}
          </Link>
        </div>
